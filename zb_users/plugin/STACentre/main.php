...
if (count($_POST) > 0) {
    ...
    $zbp->option['ZC_PIC_REGEX'] = trim(GetVars('ZC_PIC_REGEX', 'POST'));   //图片伪静态数据读取储存，PICTAG
    $zbp->SaveOption();
...
$ua = array(
    ...
    //pic图片路由数组，PICTAG
    'ZC_PIC_REGEX' => array(
        '{%host%}?pic={%id%}&page={%page%}',
        '{%host%}index.php/pic-{%id%}_{%page%}.html',
        '{%host%}pic-{%id%}_{%page%}.html',
        '{%host%}pic/{%id%}/{%page%}/',
        '{%host%}pic/{%alias%}/{%page%}/',
    ),
...
<!--图片路由，在路由设置页面的位置和值，PICTAG-->
<tr>
    <td><p align='left'><b>·图片的URL配置</b></p></td>
    <td><input id='ZC_PIC_REGEX' name='ZC_PIC_REGEX' style='width:500px;' type='text' value='<?php echo $zbp->option['ZC_PIC_REGEX']?>'></td>
</tr>
<tr>
    <td></td>
    <td><?php OutputOptionItemsOfUrl('ZC_PIC_REGEX')?></td>
</tr>
