...
function tpure_ZBvideoLoad(&$template){
    ...
    $article = $template->GetTags('article');
    $videostr = '';
    $videostr .= '<p class="videobox">';
    if(strpos($article->Metas->video,'.mp4')){
        $videostr .= '<video src="'. $article->Metas->video .'" controls="controls"'.($article->Metas->videoautoplay == '1' ? ' autoplay="autoplay"' : '').($article->Metas->videoloop == '1' ? ' loop="loop"' : '').'></video>';
...
function tpure_CustomCode(){
	...
	$footeradcode = $zbp->Config('tpure')->PostFOOTERADCODE;	//添加页尾广告代码，获取数据库中config的PostFOOTERAD数据
    $zbp->header .= $headercode . "\r\n";
    $zbp->footer .= $footercode . "\r\n";
	$zbp->footerad .= $footeradcode . "\r\n";	//把变量$footerad赋予$footerad
}
...
function tpure_DefaultTemplate(&$template){
    if(($template->GetTags('type') == 'index' && $template->GetTags('page') != '1') || 
        ($template->GetTags('type') == 'category' && $template->GetTags('category')->Template != 'catalog') || 
        ($template->GetTags('type') == 'tag' && $template->GetTags('tag')->Template != 'catalog') || 
        $template->GetTags('type') == 'date')
    {
        $template->SetTemplate('catalog');
    }
    if($template->GetTags('type') == 'author' && $template->GetTags('author')->Template != 'catalog'){
...
function tpure_color(){
    ...
    $skin .= ".post .readmore:hover,.post .tags a:hover,.telesmore i,.pagebar .now-page,.cmtpagebar .now-page,.pagebar a:hover,.cmtpagebar a:hover,a.backlist:hover,.cmtsubmit button:hover,#divTags ul li a:hover,#divCalendar td a,#divContorPanel .cp-login a:hover,#divContorPanel .cp-vrs a:hover,.goback:hover,.night .post .readmore:hover,.night #divContorPanel .cp-login a:hover, .night #divContorPanel .cp-vrs a:hover,.night #cancel-reply,.night .pages a:hover,.night .cmtsubmit button:hover,.night .errorpage .errschbtn,.night .errorpage .goback:hover,.closesite h1,.signuser.normal:hover,.sign span a,.searchnull a {border-color:#{$color}}"; //border-color
...
<p class="videobox"><label for="video">自动播放：<input type="text" name="meta_videoautoplay" class="checkbox" value="' . $object->Metas->videoautoplay . '" />　|　循环播放：<input type="text" name="meta_videoloop" class="checkbox" value="' . $object->Metas->videoloop . '" /> （自动和循环播放仅支持MP4视频）</label><span><input type="text" name="meta_video" id="video" placeholder="请点击右侧上传按钮选择MP4视频或手动输入站外视频通用地址..." value="' . htmlspecialchars($object->Metas->video) . '" class="metasrc"/></span><span><input type="button" class="uploadfile button" value="上传MP4" /></span></p>
            </div>';
        }
}
...
function tpure_DelModule(){
    global $zbp;
    $modules = array('tpure_hotviewarticle', 'tpure_hotcmtarticle', 'tpure_newarticle', 'tpure_recarticle', 'tpure_newcomment', 'tpure_user', 'tpure_readers');
    $w = array();
    ...
    if ($archives) {
        ...
        foreach ($archives as $monthItem) {
            ...
            foreach ($monthItem['articles'] as $item) {
                $str .= '<li><span class="archivetime">';
                if($zbp->Config('tpure')->PostARCHIVEDATETYPE == '0'){
                    $str .= '['.$item->Time('m/d').']';
                }else{
                    $str .= '['.$item->Time('m月d日').']';
                }
                $str .= '</span> <h3><a href="'.$item->Url.'" title="'.$item->Title.'">'.$item->Title.'</a></h3></li>';
            }
            $str .= '</ul>';
            $str .= '</div>';
        }
...
function tpure_Thumb($Source, $IsThumb = '0'){
    ...
    if ($zbp->Config('tpure')->PostIMGON == '1'){
        if (isset($Source->Metas->proimg)) {
            ...
            if($zbp->Config('tpure')->PostTHUMBON == '1' || $IsThumb == '1'){
                $temp = $zbp->Config('tpure')->PostTHUMB;
            }elseif($zbp->Config('tpure')->PostRANDTHUMBON == '1' || $IsThumb == '2'){
...
function tpure_Thumb_new($Source, $IsThumb){
    ...
    if ($zbp->Config('tpure')->PostIMGON == '1'){
        if (isset($Source->Metas->proimg)) {
            $temp = $Source->Metas->proimg;
        }elseif($Source->ImageCount >= 1 && (count($thumbs = $Source->Thumbs(120, 84, 1)) > 0)){
            $temp = $thumbs[0];
        }else{
            if($zbp->Config('tpure')->PostTHUMBON == '1' || $IsThumb == '1'){
                $temp = $zbp->Config('tpure')->PostTHUMB;
...
function tpure_LazyLoadReplace($content){
    global $zbp;
    $placeIMG = $zbp->host . "zb_users/theme/tpure/style/images/lazyloading.gif";
    $pattern = "/<img([^>]+)src=[\"']([^\"']+)[\"']([^>]+)>/";
...
function tpure_Config(){
    ...
    $array = array(
        ...
        'PostPAGEINFO' => '{"user":"1","date":"1","view":"0","cmt":"0","edit":"1","del":"1"}',
        'PostSINGLEKEY' => '1',
        'PostPAGEKEY' => '1',
        'PostRELATEON' => '1',
        'PostRELATECATE' => '1',
