...
{if $zbp->Config('tpure')->PostLOGOON == '1'}
        <div class="logo{if $zbp->Config('tpure')->PostLOGOHOVERON=='1'} on{/if}"><a href="{$host}"><img src="{if $zbp->Config('tpure')->PostLOGO}{$zbp->Config('tpure')->PostLOGO}{else}{$host}zb_users/theme/{$theme}/style/images/logo.svg{/if}" alt="{$name}" /></a></div>
{else}
