...
class Template
{
    ...
    protected function addNonexistentTags()
    {
        ...
        }	//判断页面是否有{$header}，没有自动添加
		
		if (strpos($templates['footerad'], '{$footerad}') === false) {
            if (strpos($templates['footerad'], '</body>') !== false) {
                $templates['footerad'] = str_replace('</body>', '{$footerad}' . '</body>', $templates['footerad']);
            } elseif (strpos($templates['footerad'], '</html>') !== false) {
                $templates['footerad'] = str_replace('</html>', '{$footerad}' . '</html>', $templates['footerad']);
            } else {
                $templates['footerad'] = '{$footerad}' . $templates['footerad'];
            }
        }  */	//判断页面是否有{$footerad}，没有自动添加
    ...
    public function MakeTemplateTags()
    {
		...
		$this->templateTags['footerad'] = &$zbp->footerad;		//添加template目录下footerad名字进zbp->themeinfo
